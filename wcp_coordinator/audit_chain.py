...
The audit chain is the shared substrate between the human-contractor path and
the robot path. Every state transition (per spec/0.1.md Section 1) emits one
signed AuditChainEntry whose `this_hash` becomes the next entry's `prev_hash`.

Integrity property: given any starting `this_hash`, walking backward through
`prev_hash` reaches the genesis of the chain for a given (task_id, claim_id);
...
class AuditSigner:
    """Signs audit chain entries with the coordinator's key.
