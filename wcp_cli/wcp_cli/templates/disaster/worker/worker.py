"""WCP worker scaffold: {{NAME}} ({{CLASS}}, domain=disaster).

Domain: disaster response.
Typical use cases: search-and-rescue drones, damage-survey teams, supply-route reconnaissance.

Adjacent domains using the same protocol: emergency, infrastructure.
"""
...
@worker.capability(
    ...
    certifications=[],
    class_extension={"domain": "disaster"},
)
def declare() -> None:
...
async def handle_primary(task: dict) -> dict:
    ...
    return {"completed_at": datetime.now(timezone.utc).isoformat(), "domain": "disaster"}
