"""WCP worker scaffold: {{NAME}} ({{CLASS}}, domain=manufacturing).

Domain: manufacturing dispatch.
Typical use cases: tool-change ops, line-side material runs, QC sample pulls.

Adjacent domains using the same protocol: industrial, logistics.
"""
...
@worker.capability(
    ...
    certifications=[],
    class_extension={"domain": "manufacturing"},
)
def declare() -> None:
...
async def handle_primary(task: dict) -> dict:
    ...
    return {"completed_at": datetime.now(timezone.utc).isoformat(), "domain": "manufacturing"}
