"""WCP worker scaffold: {{NAME}} ({{CLASS}}, domain=research).

Domain: research field operations.
Typical use cases: environmental sample collection, sensor-node maintenance, expedition resupply.

Adjacent domains using the same protocol: scientific, agriculture.
"""
...
@worker.capability(
    ...
    certifications=[],
    class_extension={"domain": "research"},
)
def declare() -> None:
...
async def handle_primary(task: dict) -> dict:
    ...
    return {"completed_at": datetime.now(timezone.utc).isoformat(), "domain": "research"}
